...
import com.kriaga.verticalslabs.items.VerticalPolishedBlackstoneBrickSlabItem;
...
public class VerticalPolishedBlackstoneBrickSlab extends VerticalSlabBase {
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (hit.getFace().getOpposite() == state.get(FACING) &&
                !state.get(FULL) &&
                player.getHeldItem(handIn).getItem() instanceof VerticalPolishedBlackstoneBrickSlabItem) {
